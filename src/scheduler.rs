...
use dnp3::app::measurement::{AnalogInput, AnalogOutputStatus, BinaryInput, BinaryOutputStatus, Counter, DoubleBitBinaryInput, Flags, FrozenCounter, OctetString};
use dnp3::outstation::database::{Database, Update, UpdateOptions};
use dnp3::outstation::OutstationHandle;

use crate::util::{get_current_time, generate_random_bool, generate_random_double_bit, generate_random_float, generate_random_int, generate_random_string};
...
fn update_binary_input_value(db: &mut Database) {
    ...
    if is_random_update {
        let dnp3_binary_input_total = env::var("DNP3_BINARY_INPUT_TOTAL").unwrap().parse::<u16>().unwrap() - 1;
...
fn update_binary_output_value(db: &mut Database) {
    ...
    if is_random_update {
        let dnp3_binary_output_total = env::var("DNP3_BINARY_OUTPUT_TOTAL").unwrap().parse::<u16>().unwrap() - 1;
...
fn update_analog_input_value(db: &mut Database) {
    ...
    if is_random_update {
        let dnp3_analog_input_total = env::var("DNP3_ANALOG_INPUT_TOTAL").unwrap().parse::<u16>().unwrap() - 1;
        let analog_input_index = generate_random_int(0u32, dnp3_analog_input_total as u32) as u16;

        let dnp3_analog_input_value: Vec<Vec<f64>> = serde_json::from_str(env::var("DNP3_ANALOG_INPUT_RANGE").unwrap().as_str())
...
fn update_analog_output_value(db: &mut Database) {
    ...
    if is_random_update {
        let dnp3_analog_output_total = env::var("DNP3_ANALOG_OUTPUT_TOTAL").unwrap().parse::<u16>().unwrap() - 1;
        let analog_output_index = generate_random_int(0u32, dnp3_analog_output_total as u32) as u16;

        let dnp3_analog_output_value: Vec<Vec<f64>> = serde_json::from_str(env::var("DNP3_ANALOG_OUTPUT_RANGE").unwrap().as_str())
        ...
        db.update(
            ...
            &AnalogOutputStatus::new(
                generate_random_float(dnp3_analog_output_low.into(), dnp3_analog_output_high.into()),
                Flags::ONLINE,
                get_current_time(),
            ),
            UpdateOptions::detect_event(),
...
fn update_double_bit_binary_input(db: &mut Database) {
    ...
    if is_random_update {
        let dnp3_double_bit_binary_input_total = env::var("DNP3_DOUBLE_BIT_BINARY_INPUT_TOTAL").unwrap().parse::<u16>().unwrap() - 1;
        let double_bit_binary_input_index = generate_random_int(0u32, dnp3_double_bit_binary_input_total as u32) as u16;
...
fn update_counter(db: &mut Database) {
    ...
    if is_random_update {
        let dnp3_counter_total = env::var("DNP3_COUNTER_TOTAL").unwrap().parse::<u16>().unwrap() - 1;
        let counter_index = generate_random_int(0u32, dnp3_counter_total as u32) as u16;

        let dnp3_counter_value: Vec<Vec<u32>> = serde_json::from_str(env::var("DNP3_COUNTER_RANGE").unwrap().as_str())
...
fn update_frozen_counter(db: &mut Database) {
    ...
    if is_random_update {
        let dnp3_frozen_counter_total = env::var("DNP3_FROZEN_COUNTER_TOTAL").unwrap().parse::<u16>().unwrap() - 1;
        let frozen_counter_index = generate_random_int(0u32, dnp3_frozen_counter_total as u32) as u16;

        let dnp3_frozen_counter_value: Vec<Vec<u32>> = serde_json::from_str(env::var("DNP3_FROZEN_COUNTER_RANGE").unwrap().as_str())
